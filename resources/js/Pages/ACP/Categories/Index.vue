<template>
  ...
  <h1 class="mb-8 text-3xl font-bold text-zinc-700">Categories</h1>
  ...
  <div class="mb-6 flex justify-between items-center">
    ...
    <Link href="/acp/categories/crear" class="px-6 py-3 text-sm font-bold text-white leading-4 rounded shadow bg-zinc-700 whitespace-nowrap hover:bg-zinc-800 focus:outline-none focus:ring focus:ring-zinc-700/20">
      <span>Crear</span> <span class="hidden md:inline">Categoria</span>
    </Link>
  </div>

  <Table
    :header="getCategoriesHeaders"
...
<script setup>
  ...
  const getCategoriesHeaders = computed(() => {
    return [
      { label: "Id" },
      { label: "Nom" },
      { label: "Enllaç" },
  ...
  const getCategoriesRows = computed(() => {
    return props.categories.data.map(category => {
      return {
        href: `/acp/categories/${category.id}/editar`,
        values: [
          category.id,
          category.name,
          `/${category.slug}`,
  ...
  watch(search, debounce(function (value) {
    Inertia.get("/acp/categories", { search: value }, {
      preserveState: true,
      replace: true,
    });
  }, 500));
