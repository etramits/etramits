<template>
  ...
  <h1 class="mb-8 text-3xl font-bold text-zinc-700">Rols</h1>
  ...
  <div class="mb-6 flex justify-between items-center">
    ...
    <!-- <Link href="/acp/roles/crear" class="px-6 py-3 text-sm font-bold text-white leading-4 rounded shadow bg-zinc-700 whitespace-nowrap hover:bg-zinc-800 focus:outline-none focus:ring focus:ring-zinc-700/20">
      <span>Crear</span> <span class="hidden md:inline">Rol</span>
    </Link> -->
  </div>

  <Table
    :header="getRolesHeaders"
    :rows="getRolesRows"
...
<script setup>
  ...
  const getRolesHeaders = computed(() => {
    return [
      { label: "Id" },
      { label: "Nom" },
      { label: "Acrònim" },
  ...
  const getRolesRows = computed(() => {
    return props.roles.data.map(role => {
      return {
        href: `/acp/rols/${role.id}/editar`,
        values: [
          role.id,
          role.label,
          `<span class="px-2 py-1 text-xs font-bold bg-zinc-200 rounded">${role.acronym}</span>`,
          role.visible
  ...
  watch(search, debounce(function (value) {
    Inertia.get("/acp/roles", { search: value }, {
      preserveState: true,
      replace: true,
    });
  }, 500));
